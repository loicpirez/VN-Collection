...
| RESPA-003 | HIGH | Provider refresh controls can be hover-dependent and approximately 24 px tall, making them inaccessible on touch screens. Keep the action visible and enlarge its hit area. | `src/components/StockPanel.tsx` | IN_PROGRESS |
| RESPA-004 | MEDIUM | Stock query controls use compact input heights below the touch target requirement. Increase mobile control height without inflating desktop rows. | `src/components/StockPanel.tsx` | IN_PROGRESS |
