...
| R14-RESP-005 | HIGH | The Library used a conventional row grid for cards with variable metadata heights, so every following pair waited for the tallest card in the preceding row and left irregular black holes on two-column phones. The fixed quote footer also crossed the card grid while iOS Safari changed its visual viewport during bidirectional scrolling. Use native WebKit Grid Lanes where supported, a measured one-pixel implicit-grid fallback elsewhere, keep virtualization spacers outside the packed lanes, and place the quote footer in document flow on touch layouts. | `src/components/LibraryClient.tsx`, `src/components/MasonryGridItem.tsx`, `src/components/VisualViewportAnchor.tsx`, `src/app/globals.css`, browser interaction QA | DONE_WITH_DIFF |
...
| R14-A11Y-005 | MEDIUM | The quote footer reserved enough page space but collapsed to a 20 pixel touch strip and reduced its only button to a clipped 12 pixel box until interaction. Keep the compact fixed presentation on fine desktop pointers; on phone and tablet, expose a 44 pixel row and refresh target in normal document flow so it remains reversible without covering content. | `src/components/QuoteFooter.tsx`, `src/components/VisualViewportAnchor.tsx`, `src/app/globals.css` | DONE_WITH_DIFF |
