...
| --- | --- | --- | --- | --- |
| R14-I18N-009 | MEDIUM | Producer refresh returned a stable VNDB-outage code, but non-English users saw only a generic error. Centralize code-specific localization with safe English diagnostics and map the producer refresh outage precisely. | producer refresh client and shared API error reader | DONE_WITH_DIFF |
| R14-I18N-010 | HIGH | Stock batch start failures exposed stable invalid-provider, queue-full, and unavailable codes, but the client discarded them and showed only a generic HTTP status outside English. Add the missing typed code, precise copy in all locales, and a localized client mapping. | stock batch API and client | DONE_WITH_DIFF |
...
| R14-TEST-003 | HIGH | Production CSP correctly upgrades insecure subresources, but the same static directive made local HTTP WebKit interaction QA rewrite every Next.js asset URL to HTTPS and fail before hydration, creating false Safari navigation failures. Suppress only that directive under the explicit write-capable QA environment, retain it for every normal production build, and fail the interaction preflight when an HTTP QA server was started without the required mode. | Next.js security headers and browser interaction preflight | DONE_WITH_DIFF |
| R14-TEST-004 | MEDIUM | Shelf interaction QA restored its width immediately after observing the optimistic CSS change, without waiting for the first queued settings write, then attributed a delayed restore to rendering. Await and validate both persistence responses while retaining the immediate CSS assertions so the check proves UI response, write ordering, and cleanup. | shelf browser interaction scenario | DONE_WITH_DIFF |
| R14-TEST-005 | MEDIUM | A legacy Round 6 checklist test silently skipped all three scenarios whenever its deleted source document was absent, leaving permanent ignored tests in every full run. Replace it with an always-active Round 14 closure-quality contract that requires substantive resolution text and an identified implementation surface. | audit report quality test | DONE_WITH_DIFF |
| R14-TEST-006 | HIGH | The final coverage gate exposed real cancellation races, abandoned shared-request cleanup, stale timer callbacks, native disclosure hydration, AliceNet polling failures, publisher overflow variants, and one nondeterministic wishlist debounce test. Add behavior-level regressions, remove only structurally unreachable guards, and run the complete PostgreSQL-backed gate. Evidence: 949 files and 9,938 tests pass with exactly 45,239/45,239 statements, 38,410/38,410 branches, 9,264/9,264 functions, and 38,648/38,648 lines covered; no ignore or threshold workaround was added. | VNDB/EGS request clients, VN detail and stock components, lifecycle tests, wishlist tests | DONE_WITH_DIFF |
| R14-TEST-007 | HIGH | The cancellation-aware network refactor added six unexercised instructions across the immediate proxy-abort, signaled DNS-rejection, and delayed queue-wakeup paths, while defensive waiter guards encoded states that the single-threaded FIFO cannot reach. Add behavioral regressions for every real path, simplify the guards to the queue invariant without weakening identity-safe removal, and rerun the exact PostgreSQL-backed gate. | proxy request lifecycle, DNS pinning, VNDB throttle tests | DONE_WITH_DIFF |
| R14-TEST-008 | HIGH | Responsive closure depended on route samples and screenshots that did not prove card row alignment, fixed-footer stability, localized overflow, or whether a failed preferred image really loaded its alternate source. Add one deterministic audit over all 40 page routes, five viewport classes, three browser engines, and all supported locales. Keep browser failures blocking unless a loaded fallback element proves recovery. | `scripts/responsive-audit.mjs`, QA script contracts, compiled browser reports | DONE_WITH_DIFF |
| R14-UX-082 | HIGH | A silent VNDB-status refresh could supersede the initial visible load; the stale owner correctly stopped updating state, but the silent owner never cleared the initial loading flag, leaving the VN panel permanently stuck on its skeleton. Always let the current request owner finish loading state and cover a delayed JSON body followed by a newer refresh. | `src/components/VndbStatusPanel.tsx`, `tests/VndbStatusPanel.branches.test.tsx` | DONE_WITH_DIFF |
| R14-UX-083 | HIGH | The shelf page mounts the same display controls in its normal and fullscreen surfaces. Each instance removed the shared root variables when its effect was replaced or unmounted, so one owner could erase the other owner's current width and make the rendered shelf snap back during persistence. Treat the variables as shared current display state: every live update replaces them, while an individual control unmount never clears another surface's values. | shelf display controls and browser interaction workflow | DONE_WITH_DIFF |
| R14-UX-086 | HIGH | A stale preferred local artwork path could end in the unavailable placeholder even when a valid remote source existed, while image frames exposed different loading behavior during that retry. Resolve an ordered primary and alternate source inside the shared image component, retain one skeleton through the retry, expose loaded fallback provenance for QA, and report an error only after every usable source fails. Remove the now-redundant second failure state from the VN cover wrapper. | `src/components/SafeImage.tsx`, `src/components/CoverHero.tsx`, image lifecycle tests | DONE_WITH_DIFF |
...
| R14-UX-071 | HIGH | VN metadata silently discarded every language after the twelfth while rendering every publisher inline, which hid real language filters and let records with dozens of release publishers dominate the mobile hero. Keep bounded inline summaries and expose every remaining language and publisher through accessible, keyboard-safe disclosures with their original destinations. | VN detail language and publisher metadata | DONE_WITH_DIFF |
| R14-UX-072 | HIGH | The VN route loader gave its overlapping cover and banner the same uninterrupted opaque surface, so the 2:3 cover disappeared into the 16:9 banner and looked as if the banner occupied both regions, especially under Firefox compositing. Preserve one shared pulse while clipping the banner independently and giving the overlapping cover its final border, radius, aspect ratio, stacking, and shadow. | VN route hero loading boundary | DONE_WITH_DIFF |
| R14-UX-073 | MEDIUM | Inline character metadata spoilers placed their state marker on the gated button, then removed that node after a persistent reveal. Focus, assistive state inspection, and interaction tests could retain a detached transient element even though the value was visible. Keep one stable state wrapper across the button, revealed value, and hide action. | `src/components/CharacterMetaClient.tsx` | DONE_WITH_DIFF |
| R14-UX-074 | HIGH | Once the VN server tree resolved, banner and cover image placeholders started independent colour animations. Their phases diverged across browsers, so the overlapping banner brightened through the cover area and the hero looked structurally unstable. Drive both placeholders from one inherited hero colour clock, retain the cover boundary in loaded and loading states, and disable the shared motion when reduced motion is requested. | VN hero banner, cover, route loader, shared skeleton styles | DONE_WITH_DIFF |
| R14-UX-075 | HIGH | Remote custom cover URLs on collection cards were passed through the local-file slot, producing malformed `/api/files/https:/...` requests and leaving cards on their unavailable-image state even though a valid remote cover existed. Classify custom artwork by scheme, keep uploaded paths local, and pass remote overrides through the remote source contract. | `src/components/VnCard.tsx` | DONE_WITH_DIFF |
| R14-UX-076 | MEDIUM | Duplicate release-image entries reused the same React key and rendered the same artwork twice, producing unstable reconciliation warnings during navigation. Remove only exact image duplicates by stable identity while retaining distinct package faces that happen to share a source identifier. | release detail media gallery | DONE_WITH_DIFF |
| R14-UX-077 | HIGH | VN artwork pickers keyed release images only by release and source id, so different package faces or repeated rows collided as `pkg-51` and React could duplicate or omit picker tiles. Share one complete release-image identity and exact deduplication contract across the media gallery, cover picker, and banner picker. | VN media and artwork picker dialogs | DONE_WITH_DIFF |
| R14-UX-078 | HIGH | The root loading boundary reordered prebuilt home-section elements without assigning keys, so every route transition emitted a server-render warning and React could reconcile destination skeleton sections inconsistently across concurrent tabs. Wrap every ordered section in a stable keyed fragment without changing the loading geometry. | application root loading skeleton | DONE_WITH_DIFF |
| R14-UX-079 | HIGH | Character detail pages exposed dense metadata and trait links through default Next.js prefetching, launching a large burst of speculative RSC requests that delayed hydration and left visible spoiler controls temporarily inert; React selective hydration could then consume the first hover or tap without revealing anything. Disable speculative prefetch for dense character destinations and use a CSS-previewed native disclosure for gated trait chips so hover, keyboard, and the first tap work before hydration while normal navigation remains unchanged after reveal. | character detail links and spoiler trait chips | DONE_WITH_DIFF |
| R14-UX-080 | HIGH | Eroge Price package rows used only edition and retailer id as their React key, but one retailer can legitimately sell several distinct product references for the same VN. Preserve every offer and identify each rendered row with edition, retailer, product identity, and its stable source position so reconciliation cannot duplicate or omit prices. | VN Eroge Price retailer lists | DONE_WITH_DIFF |
| R14-UX-081 | HIGH | WebKit speculatively prefetched every visible recently checked Stock link as an RSC request behind Basic Auth. The unused requests failed access-control checks, added authentication challenges, and could contribute to the repeated Safari login symptom before the operator selected any row. Prefetch is disabled only for these bounded activity links while direct navigation remains unchanged. Deployed Chromium and WebKit checks observe zero idle RSC prefetches, zero Stock response failures, zero browser errors, one main landmark, and no horizontal overflow before a successful VN Stock navigation. | recent Stock activity navigation | DONE_WITH_DIFF |
| R14-UX-084 | MEDIUM | The VN media lightbox exposed previous and next buttons plus keyboard arrows, but mobile users could not move between images with the expected direct swipe gesture. Add touch-only horizontal gesture recognition with a deliberate threshold, vertical-motion rejection, first/last wrapping, preserved pinch and vertical touch behavior, and complete branch tests. | VN media lightbox | DONE_WITH_DIFF |
| R14-UX-085 | MEDIUM | Cast cards stacked two independently aligned links plus separate original-name text next to an 80-pixel portrait, creating excessive and inconsistent vertical gaps. Integrate the original name into the character target, retain two full 44-pixel touch targets, align their text toward the shared boundary, tighten line height, and retain independent character and staff destinations. | VN cast and seiyuu cards | DONE_WITH_DIFF |
| R14-UX-087 | MEDIUM | The collapsed mobile quote dock kept its refresh action visible, but activating it fetched a new quote without expanding the hidden content. Open the dock before refreshing, preserve explicit collapse, and execute the complete refresh and toggle sequence in real touch contexts under Chromium and WebKit. | quote dock interaction and browser QA | DONE_WITH_DIFF |
| R14-FEAT-003 | HIGH | Shelf `contain/cover` preference wrote `object-fit` to the outer `SafeImage` wrapper even though that property is not inherited by the nested image. The exposed setting therefore had no effect while unsafe `as never` casts hid the target mismatch. Add an explicit inner-image class contract, consume the CSS variable on the native image, retain scaling on the wrapper, and test both consumers. | spatial shelf cards, face-out displays, shared safe image contract | DONE_WITH_DIFF |
...
| R14-RESP-004 | HIGH | WebKit could dispatch a desktop or mobile navigation click against the server-rendered button before React attached its handler, producing a visible no-op that looked like a broken navbar. Expose an honest disabled/busy state until hydration and make browser QA wait for the control to become interactive before asserting menu geometry. | grouped desktop and mobile navigation | DONE_WITH_DIFF |
| R14-RESP-005 | HIGH | A mobile card-gap correction incorrectly changed the Library from ordered rows to independent masonry lanes. That made adjacent cards start at different vertical positions and broke row scanning. The same correction moved the quote footer into document flow, so it disappeared until the end of a long virtualized Library. Restore a deterministic CSS row grid while retaining each card's natural height, remove all measured row spans and Grid Lanes behavior, and keep the quote dock native-fixed without scroll or visual-viewport position writes. Browser QA must assert equal top coordinates for every visible card pair, a full-width mobile dock, a stable top coordinate after down and reverse scrolling, and a reversible touch toggle in both WebKit and Chromium. | `src/components/LibraryClient.tsx`, `src/components/QuoteFooter.tsx`, `src/app/globals.css`, browser interaction QA | DONE_WITH_DIFF |
| R14-RESP-006 | HIGH | Residual touch layouts still forced density controls into one row at the 640-pixel breakpoint, exposed edition checkboxes through only the native 16-pixel box, and compressed AliceNet pagination into an overflowing inline group. Let density controls wrap at touch widths while preserving a 44-pixel range target, wrap native checkboxes in 44-pixel labels, and stack pagination into two bounded rows before the small-screen breakpoint. | density settings, owned-edition controls, AliceNet shop pagination | DONE_WITH_DIFF |
| R14-RESP-007 | HIGH | Series detail, Top-ranked, Upcoming, Shelf, Tags, and Traits kept identity text and wide density or refresh controls in one row at phone and landscape widths. Long titles collapsed into narrow columns or collided with actions even though the document itself did not overflow. Stack identity and toolbar regions until the large breakpoint, retain bounded full-width controls, and mirror the same geometry in every route skeleton. | six page headers and matching loading boundaries | DONE_WITH_DIFF |
| R14-A11Y-001 | HIGH | Recheck landmarks, headings, names, labels, focus order, keyboard operation, dialogs, announcements, image alternatives, color-independent state, reduced motion, and target sizing across every route and major interaction. | application-wide | DONE_WITH_DIFF |
...
| R14-A11Y-005 | MEDIUM | The quote footer previously collapsed to an undersized touch strip, then a later mobile workaround placed the corrected 44 pixel controls at the end of document flow where they were no longer persistently available. Keep the 44 pixel expand and refresh targets on touch layouts, preserve compact fine-pointer rendering, expose a reliable expanded/collapsed state, and anchor the full-width mobile dock with native fixed positioning instead of scroll-driven coordinate updates. | `src/components/QuoteFooter.tsx`, `src/app/globals.css` | DONE_WITH_DIFF |
| R14-A11Y-006 | HIGH | The characters route rendered a density wrapper as `<main>` inside the root layout's canonical `<main id="main-content">`, creating two main landmarks in every engine and locale. Keep density scoping on a neutral wrapper and enforce one application-owned main landmark across every route source. | characters route and root landmark contract | DONE_WITH_DIFF |
| R14-A11Y-007 | HIGH | Shared `tap-target` utilities left controls with 10, 24, or 28 pixel layout boxes and enlarged only an overlapping pseudo-element; dense card actions, tag removal, EGS links, colour pickers, and add controls could therefore steal adjacent taps while still reporting undersized geometry. Give both utilities real 44-pixel boxes on touch layouts, compact only for a fine pointer at desktop width, remove overlapping pseudo-elements, and keep colour swatches as compact inner visuals. | global touch utility and all shared consumers | DONE_WITH_DIFF |
| R14-A11Y-008 | HIGH | The production route matrix found destination controls that bypassed the shared touch contract: tiny R18 thumbnails in reading and anniversary strips, wishlist card Add overlays, Data and Schema disclosures, reading-goal actions, and VN staff/tracking chips. It also found character image links whose name disappeared while `SafeImage` deferred the native image. Preserve 44-pixel boxes on touch devices, compact only for a fine hover pointer, and give image links a stable explicit name independent of image loading. | home strips, VN cards/detail, Data, Schema, Stats, Cast and Staff sections | DONE_WITH_DIFF |
| R14-A11Y-009 | HIGH | The post-deployment touch audit isolated additional controls whose own hit box remained below 44 pixels: EGS icon actions and shop links, tag-overlap rows, staff-role links, aspect choices, VN similarity navigation, the physical-location editor, series links, collection removal, and wishlist filter reset. Nested checkboxes and search inputs whose enclosing label already provides a 44-pixel target are valid and must not be inflated independently. Give standalone controls a real 44 by 44 target on touch layouts and retain compact density only with a fine hover pointer. | VN detail metadata and edit surfaces, EGS details, wishlist filters | DONE_WITH_DIFF |
| R14-A11Y-010 | MEDIUM | The final production WebKit matrix found maintenance refresh actions only 32 pixels wide and the schema search input only 22 pixels high on touch, despite their enclosing rows being otherwise responsive. Give both controls a 44-pixel mobile interaction envelope and restore compact dimensions only on fine-hover layouts. | data maintenance and schema browser | DONE_WITH_DIFF |
| R14-A11Y-011 | MEDIUM | The Stock panel's short `All` group label produced a 41-pixel button in phone landscape across Chromium, Firefox, and WebKit, leaving that one filter below the shared touch target contract. Give every Stock group filter a 44-pixel minimum width and pin the requirement in source and three-engine responsive QA. | VN Stock panel group filters | DONE_WITH_DIFF |
| R14-I18N-001 | HIGH | Recheck French, English, and Japanese dictionary parity, hardcoded visible strings, date/time and number formatting, platform names, plural/range text, metadata, error messages, and layout resilience under longer translations. | i18n dictionaries and all rendered surfaces | DONE_WITH_DIFF |
...
| R14-I18N-008 | MEDIUM | Tag metadata used the raw route id as the browser title on a cold API cache, then changed to the tag name only after client hydration populated the cache. Resolve cache-only names from both API and web-detail snapshots and use a localized neutral fallback rather than exposing an internal id or blocking the page on VNDB. | tag detail metadata and dictionaries | DONE_WITH_DIFF |
| R14-SEC-001 | CRITICAL | Re-audit authentication gates, mutation authorization, CSRF/origin handling, SSRF and URL allowlists, uploads and path traversal, request size limits, secret/error exposure, CSP and headers, proxy behavior, dependencies, and production TLS/reverse-proxy configuration. | all APIs, middleware, Next and production configuration | DONE_WITH_DIFF |
...
| R14-SEC-004 | MEDIUM | The Nginx Basic Auth password file was mode `0644`, allowing unrelated local accounts to copy the password hash for offline attack. Restrict it to `root:www-data` mode `0640`, verify worker readability, retain root-only proxy-proof material, and document the permission contract for every rotation. | production Basic Auth file and reverse-proxy operations | DONE_WITH_DIFF |
| R14-SEC-005 | HIGH | Concurrent Safari and WebKit tabs could race separate Basic Auth challenges, leaving background RSC and API requests with intermittent `401` responses even after the main document authenticated. Bridge the first successful Basic Auth response into a seven-day root-token-backed `Secure`, `HttpOnly`, `SameSite=Strict` browser session, retain the challenge for missing or invalid cookies, prevent failed credentials from minting a cookie, and verify multi-tab navigation without weakening page or API protection. | production Nginx authentication and browser sessions | DONE_WITH_DIFF |
| R14-FEAT-001 | HIGH | Exercise complete library, wishlist, search, filter/group/sort, collection mutation, compare, shelf, release/edition, lists, series, staff, downloads, backups, and settings workflows, including immediate state refresh and failure recovery. | core product workflows | DONE_WITH_DIFF |
...
| R14-STOCK-009 | HIGH | The VN stock panel applied its physical-provider default in a follow-up effect keyed only by provider count. The live snapshot could render with every provider selected and leave the refresh action on the generic scope depending on effect ordering. Apply the physical selection atomically with the decoded live snapshot, while retaining the explicit all-provider scope on the generic Stock page. | per-VN stock panel provider selection | DONE_WITH_DIFF |
...
| R14-ALICE-002 | HIGH | AliceNet operation buttons were rendered as enabled before the client attached their handlers, so an immediate click during shop hydration appeared to do nothing and started no background job. Keep the complete shop toolbar visible, disable its four operation controls until hydration completes, retain the existing starting-state lock, and prove that the compiled interaction launches a detached job with determinate progress and a stop action. | AliceNet shop client and compiled browser interaction workflow | DONE_WITH_DIFF |
...
| R14-PERF-001 | HIGH | Recheck bounded queries, pagination/virtualization, tag indexes, repeated repository calls, client polling, background jobs, multi-tab behavior, images, DOM size, bundle boundaries, memory, database pool pressure, and slow provider isolation. | application and production runtime | DONE_WITH_DIFF |
...
| R14-PERF-003 | MEDIUM | Opening the synthetic-entry VNDB linker issued the seeded lookup immediately and scheduled the same lookup again 300 ms later; closing the dialog could also leave that delayed request alive. Run one immediate lookup from the explicit open action, debounce only subsequent query changes, and cancel the pending timer whenever the query, identity, or open state changes. | `src/components/LinkToVndbButton.tsx` | DONE_WITH_DIFF |
| R14-PERF-004 | HIGH | VN detail consumers canceled their own state updates when a tab closed, but shared browser and server requests retained abandoned subscribers in the VNDB throttle, cache, retry waits, and EGS fetch chain. Rapid multi-tab navigation therefore accumulated stale upstream work for several seconds and could delay or fail the active page. Shared requests now track independent consumers, remove canceled work from queues and backoff timers, abort the upstream request only when its last consumer leaves, and propagate the route request signal through VNDB and EGS detail calls. A compiled-browser stress run opened eight VN tabs, closed seven, retained one usable page, and loaded a follow-up page with status 200 in 5.09 seconds without browser or response failures; the server used about 162 MiB afterward. The focused suite passes 291 tests and the production build succeeds. | VN detail read routes, VNDB and EGS clients, shared request caches and throttle | DONE_WITH_DIFF |
| R14-PERF-005 | HIGH | Concurrent production VN renders could hydrate the dynamically imported tracking-route section before React finished replacing the nested server skeleton placeholders. Under load this removed three `P:*` targets and caused repeated `$RS` `parentNode` exceptions in Chromium. Import the route section statically while retaining its own destination-shaped data-loading skeleton, and pin the single-boundary contract. | VN detail route section loading and multi-tab rendering | DONE_WITH_DIFF |
| R14-PERF-006 | CRITICAL | The VNDB throttle removed its next waiter from the FIFO before dispatching it on a zero-delay timer. If that caller was canceled in the intervening task, its cleanup used an absent index and removed the last unrelated waiter; the already-scheduled canceled callback could then acquire a slot whose rejected caller would never release it. Replace callback handoff with one central FIFO pump, let only the queue head own a delay, remove waiters by identity, and wake the next request after every cancellation. Bound automatically loaded VN panels across queue, proxy setup, DNS validation, and network time while leaving background batches on their independent lifecycle. | VNDB throttle, provider fetch, DNS pinning, and automatic VN detail routes | DONE_WITH_DIFF |
| R14-PERF-007 | CRITICAL | Dense library, shelf, discovery, producer, staff, search, and shop surfaces let Next.js automatically prefetch every visible VN detail link. Opening the Library alone issued and then canceled many expensive VN RSC renders, competing with hydration and making early controls appear inert. Disable speculative prefetch for VN detail links on every audited dense or global surface while preserving ordinary click navigation, and enforce the policy structurally. | VN cards, home strips, shelves, discovery grids, search and shop result lists | DONE_WITH_DIFF |
| R14-PERF-008 | HIGH | The VNDB status route awaited label discovery before starting the entry request and local collection lookup, so a healthy entry could inherit queue delay from an unrelated label call and exceed the interactive deadline. Start labels, entry, and local state concurrently under the same bounded request signal, and give the complete interactive operation a 30-second budget without changing background job lifecycles. | VNDB status route and request deadline contracts | DONE_WITH_DIFF |
| R14-PERF-009 | CRITICAL | The optional anime chip was an asynchronous server component whose uncached VNDB search could keep the VN document stream open until the upstream timeout. One slow optional badge therefore blocked `DOMContentLoaded` and multiplied across Safari tabs even though the primary VN data was already available. Render fresh or stale cached matches immediately, refresh stale positive data in the background, and cap a cold search contribution to 350 milliseconds while allowing the shared cache request to finish independently. | `src/components/AnimeChip.tsx`, VN detail server stream | DONE_WITH_DIFF |
| R14-PERF-010 | HIGH | Character, release, and quote sections started their remote requests as soon as a VN page mounted even when they were several screens below the viewport. These requests occupied the browser connection pool and could queue an immediately selected cover or banner mutation behind unrelated reads. Activate each section once it enters a 600-pixel preload margin, preserve its destination-shaped skeleton before activation, and fall back to immediate activation when IntersectionObserver is unavailable. | VN character, release, and quote sections; shared near-viewport hook | DONE_WITH_DIFF |
| R14-DATA-001 | CRITICAL | PostgreSQL 16.15 is loopback-only with checksums enabled and a 50-connection ceiling. Production records all 11 migrations and 58 public tables, with zero invalid constraints, invalid indexes, or quarantined JSON rows. Application, migration, and backup identities are non-superusers; the backup identity is read-only and the web pool remains capped at 10. Fresh database and media archives pass their checksums and isolated restore drills reproduced exact database and media counts before cleanup. The final repository gate passes all 95 PostgreSQL integration scenarios, while the complete PostgreSQL-backed coverage run exercises the shared SQLite and PostgreSQL contracts at exactly 100% statements, branches, functions, and lines. | PostgreSQL repositories, migrations, production database, backup services | DONE_WITH_DIFF |
| R14-TYPE-001 | HIGH | Re-scan production and test code for weakened types, unsafe casts, suppression directives, unvalidated external payloads, and exported contracts lacking useful documentation. | `src`, `tests`, `scripts` | DONE_WITH_DIFF |
| R14-TEST-002 | HIGH | Run focused tests while fixing findings, then the complete unit, PostgreSQL, exact coverage, QA, interaction, sentinel, provider, browser, and production health gates. No ignored files, skipped new scenarios, or threshold workarounds. | all test and QA suites | DONE_WITH_DIFF |
| R14-DOC-001 | MEDIUM | Reconcile README, FEATURES, CLAUDE, deployment and PostgreSQL docs, active TODO reports, route/provider claims, AliceNet naming, and final verification evidence with the shipped application. | project Markdown and operational docs | DONE_WITH_DIFF |
| R14-DOC-002 | MEDIUM | The active Round 14 tracker reused three identifiers for unrelated findings, making counts and references ambiguous. Give every active task a unique identifier and enforce uniqueness across all non-archived report tables. | active TODO reports and tracker contract | DONE_WITH_DIFF |
| R14-DOC-003 | MEDIUM | AliceNet documentation still described a SQLite-only unbounded reader and live VNDB wishlist enrichment, and the generic stock contract listed only three of seven persisted provider states. Document the configured repository, bounded cached enrichment, pagination metadata, and complete status union shipped by the application. | `CLAUDE.md`, `FEATURES.md` | DONE_WITH_DIFF |
| R14-DOC-004 | MEDIUM | The developer guide still described localhost as the only deployment and read routes as permanently ungated, contradicting the shipped token and authenticated-proxy policy. The TODO index also labelled a completed Round 8 report as current and omitted its active `IN_PROGRESS` status. Align the threat model and make Round 14 the sole active report while retaining prior rounds as completed evidence. | `CLAUDE.md`, `TODO/README.md` | DONE_WITH_DIFF |
| R14-DOC-005 | MEDIUM | The TODO index classified Round 14 as completed after the responsive candidate was rolled back in production, even though its deployment verification row remained `IN_PROGRESS`. Keep Round 14 in the active index until the corrected immutable release is explicitly authorized, activated, and verified. | `TODO/README.md`, Round 14 operations status | DONE_WITH_DIFF |
| R14-OPS-001 | CRITICAL | Every Round 14 feature release was pushed and activated independently. Final closure verifies identical local, remote, and production revisions; an active release symlink and process working directory; PostgreSQL readiness with an idle bounded pool; zero service restarts or warning journal entries; loopback-only application and database listeners; authenticated public routing; successful Nginx validation; current checksum-valid database and media archives; successful backup units; retained rollback releases; and stable memory after the complete Chromium and WebKit production matrix. | production host and deployment tooling | DONE_WITH_DIFF |
...
- The production route matrix completed 108 renders across Chromium desktop,
  Firefox tablet, and WebKit mobile with HTTP 200, one main landmark, no page
  error, no console error, no duplicate id, no missing image alternative, no
  fixed or document overflow, and no broken image-link name. A follow-up WebKit
  pass verified all independently clickable VN metadata, EGS, wishlist,
  character, staff, schema, release, recommendation, and tag controls after
  excluding only hidden skip controls, enclosing 44-pixel labels, and inline
  prose links. Three isolated five-second WebKit runs then completed without a
  failed request or browser error, proving the earlier cross-navigation aborts
  were audit-harness timing rather than application failures.
...
- A production Chromium stress test reproduced the VN streaming race before
  the correction: concurrent tabs lost three tracking-route skeleton targets
  and raised repeated React `$RS` `parentNode` exceptions. After replacing the
  nested module fallback with the section's existing data fallback, four
  consecutive waves opened 24 VN tabs each. All 96 requests returned HTTP 200,
  all 96 pages exposed their title, and no browser, console, or server-response
  error was observed. Forty-four focused section and route scenarios, the
  complete 9,847-test suite, typecheck, build, deployment health, and zero
  service restarts also pass.
- Shelf display variables now remain owned by the shelf surface rather than an
  individual normal or fullscreen control instance. The component regression
  proves that unmounting one owner retains the rendered width, and the complete
  compiled-browser workflow changes, persists, and restores the width while
  all 25 interaction scenarios pass.
- VNDB request scheduling now uses one cancellation-aware FIFO pump. A targeted
  three-caller regression cancels the delayed head and proves the following
  caller still runs with no active or queued residue. Provider deadlines remain
  retained through proxy setup and DNS pinning, while automatically loaded VN
  panels have a ten-second total budget that does not alter background stock or
  download jobs. The complete suite passes 950 files and 9,856 tests; the
  compiled Chromium/WebKit workflow passes 25 of 25 scenarios, and throttle
  telemetry remains at zero active and zero queued both immediately and twelve
  seconds after browser shutdown.
- Per-VN stock loading now commits the live provider snapshot and its physical
  default in one state transition. Both independent live-load regressions pass,
  the generic Stock surface still selects all providers, and all 114 stock-panel
  scenarios plus typecheck pass.
- Immediate proxy abort, signaled DNS rejection, and duplicate delayed-wakeup
  paths now have explicit network regressions. Queue cancellation removes the
  exact waiter by identity without structurally unreachable settled/index
  branches, and the active label follows the one-request invariant. The final
  PostgreSQL-backed run passes 951 files and 9,954 tests with exactly 45,264 of
  45,264 statements, 38,413 of 38,413 branches, 9,273 of 9,273 functions, and
  38,676 of 38,676 lines covered, with no ignore or threshold workaround.
- VN detail navigation on eighteen dense or globally mounted surfaces now opts
  out of automatic Next.js prefetch. A compiled-browser network probe observed
  zero speculative `/vn/*` RSC requests during a five-second Library render,
  down from multiple simultaneous renders before the correction; direct click
  navigation is unchanged and the complete source policy has eighteen passing
  structural scenarios.
- The final application-wide browser matrix rendered all 40 App Router pages
  in Chromium, Firefox, and WebKit at 1440 by 1000 and 390 by 844 pixels in
  French. Eight critical workflows were also rendered in English and Japanese
  at both widths, for 272 total renders. Every final case returned HTTP 200,
  exposed exactly one `main#main-content`, used the requested document language,
  produced no document overflow, and contained no fatal runtime content. The
  13 requests initially interrupted by immediate cross-route navigation all
  passed when replayed in isolated pages through complete hydration.
- Navigation, settings, map stacking, AliceNet progress, artwork actions,
  shelves, tags, recommendations, spoilers, responsive VN detail, and touch
  behavior pass all 25 compiled Chromium/WebKit interaction scenarios. The
  structural browser QA and regression sentinel each pass 29 of 29 checks.
  The accessibility, responsive, i18n, typing, pagination, virtualization,
  index, and stock audit subset passes 235 focused scenarios.
- Every configured provider was called from the current workstation. Across
  the 25 supplied and canonical URLs, 20 returned a normal application or
  search page, Suruga-ya's two pages and AmiAmi returned Cloudflare protection,
  Joshin returned HTTP 403, and Bikkuri Takarajima returned only a bounded
  browser-back script. The real per-VN pipeline then exercised all 22 live
  providers in 140 seconds on the isolated QA database and returned 51 offers.
  It reported five successful structured providers, a protected partial
  Suruga-ya result, explicit bounded failures, and valid no-result or skipped
  states for the remaining providers. AliceNet remained separate and its
  controls were present only on the linked shop detail workflow.
- Final verification passes 951 ordinary test files and 9,877 tests, 95
  PostgreSQL integration scenarios, typecheck, and the production build. The
  PostgreSQL-backed coverage gate passes 952 files and 9,973 tests with exactly
  45,264 of 45,264 statements, 38,413 of 38,413 branches, 9,273 of 9,273
  functions, and 38,676 of 38,676 lines. No ignore, suppression, excluded file,
  or threshold workaround was introduced. The dependency audit covers 586
  packages and reports zero vulnerabilities.
- Mobile cast cards were compared directly with their previous rendering. The
  older card stacked two separate 44-pixel links around an additional
  original-name line, while the first correction compressed both destinations
  into 32-pixel centered blocks. The final card keeps two independent
  44-pixel touch targets in an 88-pixel text column, integrates the original
  name into the character destination, and aligns both labels toward their
  shared boundary. Character and seiyuu navigation remain independent.
- The media lightbox now recognizes a deliberate horizontal touch swipe,
  rejects short or predominantly vertical movement, wraps at both ends, and
  preserves button, keyboard, pan, and pinch behavior. WebKit-specific touch
  tracking also releases cleanly when the gesture ends outside the image
  frame.
- AliceNet operation controls remain visible only inside the linked shop, but
  cannot be activated before their client handlers exist. The compiled browser
  workflow waited for hydration, launched a detached pipeline, observed 2 of 5
  phases at 40 percent with the current item and stop action, and completed all
  25 interaction scenarios. The structural browser QA and sentinel each pass
  all 29 checks.
- Final local verification passes 951 ordinary test files and 9,885 tests, all
  95 PostgreSQL integration scenarios, strict typecheck, and the production
  build. The PostgreSQL-backed coverage gate passes 952 files and 9,980 tests
  with exactly 45,289 of 45,289 statements, 38,441 of 38,441 branches, 9,277
  of 9,277 functions, and 38,697 of 38,697 lines. No ignore, suppression,
  excluded source file, or threshold workaround was added.
- The final production browser audit passes 36 checks across Chromium and
  WebKit at 1440 and 390 pixels. Library, Wishlist, Stock, places, the linked
  AliceNet shop, map, Top Ranked, and a VN detail page each return HTTP 200
  with one main landmark, no console or page error, and no document overflow.
  AliceNet operations appear only on the linked shop, and the per-VN stock
  surface remains present. The request throttle is idle immediately and after
  twelve seconds, with no active or queued job.
- Production serves the pushed release through its versioned release target,
  reports PostgreSQL available with a pool maximum of 10 and no waiters, and
  has zero service restarts or warning journal entries. Peak memory remains
  below 166 MiB after browser QA. Nginx configuration validation succeeds;
  unauthenticated and forged-session requests return 401, a valid secure
  session returns 200 with the expected security headers, public icons return
  204, and port 3000 is unreachable externally. The latest database and media
  archives pass SHA-256 verification and archive inspection, both backup units
  report success, and eight recent versioned releases remain available for
  rollback.
- The responsive regression candidate restores an ordered CSS row grid with
  natural card heights and a native fixed quote dock. Compiled WebKit proves
  equal top coordinates for every visible card pair, stable footer coordinates
  after downward and reverse scrolling, symmetric 24-pixel panel gutters, and
  reversible touch and keyboard collapse. Chromium independently confirms the
  ordered row geometry. No measured row spans, Grid Lanes rule, visual viewport
  position writer, or artificial card height remains.
- The final French audit executed 600 route, engine, and viewport combinations.
  All 600 had one main landmark, correct language, zero overflow, zero escaped
  element, zero clipped control, zero undersized touch target, and zero geometry
  finding. Forty-seven file errors were traced exclusively to an empty QA
  storage directory; replaying all four affected routes across every engine and
  viewport against their referenced read-only storage completed 60 renders with
  zero findings. The English and Japanese critical matrix completed 144 renders;
  its one transient stale-local image event passed an exact isolated replay with
  the remote fallback loaded. In total, 805 browser renders were executed and
  all 744 unique final combinations have a passing result.
- Final local verification passes 952 ordinary test files and 9,902 tests, all
  95 PostgreSQL integration scenarios, strict typecheck, the compiled production
  build, 39 smoke checks, 28 structural browser checks, 27 Chromium and WebKit
  interactions, and 29 regression sentinel checks. The PostgreSQL-backed
  coverage gate passes 953 files and 9,997 tests with exactly 45,314 of 45,314
  statements, 38,500 of 38,500 branches, 9,285 of 9,285 functions, and 38,719
  of 38,719 lines. No ignore, suppression, source exclusion, or threshold
  workaround was added. The production dependency audit covers 297 packages
  and reports zero vulnerabilities.
- The corrected candidate passes 28 of 28 compiled Chromium and WebKit
  interactions, 29 of 29 structural browser checks, 39 of 39 smoke checks, and
  29 of 29 sentinel checks against isolated QA data. The exact WebKit build
  opens six VN documents concurrently with HTTP 200, no page or console error,
  and visible main content in 0.66 to 0.95 seconds per tab. Four final French
  phone and landscape renders across WebKit and Chromium show aligned card
  rows, a native fixed full-width quote dock, and no overflow finding.
- Fresh source verification passes 954 ordinary test files and 9,921 tests,
  all 95 PostgreSQL integration scenarios, strict typecheck, and the normal
  production build. The PostgreSQL-backed coverage gate passes 955 files and
