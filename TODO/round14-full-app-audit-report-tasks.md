...
| R14-UX-081 | HIGH | WebKit speculatively prefetched every visible recently checked Stock link as an RSC request behind Basic Auth. The unused requests failed access-control checks, added authentication challenges, and could contribute to the repeated Safari login symptom before the operator selected any row. Disable prefetch only for these bounded activity links while preserving direct navigation. | recent Stock activity navigation | IN_PROGRESS |
