...
| R14-A11Y-005 | MEDIUM | The fixed quote footer reserved enough page space but collapsed to a 20 pixel touch strip and reduced its only button to a clipped 12 pixel box until interaction. Keep the compact 20 pixel presentation on fine desktop pointers while exposing a 44 pixel row and refresh target on phone and tablet surfaces. | `src/components/QuoteFooter.tsx` | DONE_WITH_DIFF |
