...
| R14-A11Y-008 | HIGH | The production route matrix found destination controls that bypassed the shared touch contract: tiny R18 thumbnails in reading and anniversary strips, wishlist card Add overlays, Data and Schema disclosures, reading-goal actions, and VN staff/tracking chips. It also found character image links whose name disappeared while `SafeImage` deferred the native image. Preserve 44-pixel boxes on touch devices, compact only for a fine hover pointer, and give image links a stable explicit name independent of image loading. | home strips, VN cards/detail, Data, Schema, Stats, Cast and Staff sections | IN_PROGRESS |
