...
| R14-DATA-001 | CRITICAL | PostgreSQL 16.15 is loopback-only with checksums enabled and a 50-connection ceiling. Production records all 11 migrations and 58 public tables, with zero invalid constraints, invalid indexes, or quarantined JSON rows. Application, migration, and backup identities are non-superusers; the backup identity is read-only and the web pool remains capped at 10. A fresh custom-format database dump and media archive both pass their checksums. The database dump restored into an isolated database with exact key counts, and the media archive restored all 13,769 files and 1,498,187,503 bytes; both temporary restores were removed. Complete the repository PostgreSQL and SQLite-parity gates before final closure. | PostgreSQL repositories, migrations, production database, backup services | IN_PROGRESS |
