...
| FEA-002 | HIGH | Owned-edition selection is capped at 100 releases without navigation, leaving large VN release histories incomplete. Add paginated or searchable release selection. | `src/components/OwnedEditionsSection.tsx` | IN_PROGRESS |
| FEA-003 | HIGH | Generic stock offer groups render long lists without pagination or bounded expansion. Add compact pagination with range state per provider group. | `src/components/StockPanel.tsx`, `src/components/StockPricesSection.tsx` | IN_PROGRESS |
