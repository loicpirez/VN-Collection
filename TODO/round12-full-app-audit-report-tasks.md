...
| --- | --- | --- | --- | --- |
| R12-DATA-001 | CRITICAL | PostgreSQL cache statistics group by the derived `path` alias and then apply `COLLATE` to that alias in `ORDER BY`; PostgreSQL resolves the collated expression as a missing input column and production logs `42703 column path does not exist`. Order by the full `SPLIT_PART` expression or a typed subquery and add a real PostgreSQL integration assertion, not only a mocked SQL test. | `src/lib/db/repositories/cache.ts`, PostgreSQL integration suite, `/api/vndb/cache`, `/schema` | TODO |
| R12-UIUX-001 | HIGH | Data-management copy still says SQLite on PostgreSQL, and the file picker/drop receiver advertises `.db` restore even though the server correctly requires a native PostgreSQL logical backup. Expose the active backend in the diagnostics contract, show provider-neutral copy, and accept only JSON plus the current backend's native backup format. | `src/app/data/page.tsx`, `src/components/ImportPanel.tsx`, `src/components/DropImport.tsx`, i18n dictionaries | TODO |
...
| R12-I18N-002 | MEDIUM | VNDB token storage copy says the secret remains in a local SQLite database, which is false for a remote PostgreSQL deployment. Replace it with backend-neutral “application database” language in every locale and continue masking secrets in API responses. | integration settings dictionaries | TODO |
...
| R12-DOC-001 | MEDIUM | Several comments still describe persistence as SQLite-only or contain temporary implementation narration such as `commit <this one>`. Update only comments in touched ownership areas so documentation matches the dual-backend architecture without generating unrelated churn. | collection, wishlist, cover, data-management source comments | TODO |
