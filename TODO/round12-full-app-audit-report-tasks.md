...
| R12-RESP-003 | HIGH | The three data-maintenance tracks selected intrinsic grid widths from long live duplicate and stale records, expanding the 390-pixel document beyond 1,000 pixels. Both loading and resolved layouts now use zero-minimum tracks with bounded, truncating row children. Deployed WebKit and Chromium each measure a 390-pixel document at a 390-pixel viewport against the production records. | `src/components/DataMaintenance.tsx`, `/data` | DONE_WITH_DIFF |
...
| R12-ACCESS-003 | MEDIUM | The isolated mobile matrix found compact actionable targets in tag external links, brand-overlap selects, Steam links, and the Wishlist owned filter. The controls now guarantee both dimensions on touch surfaces while retaining compact pointer layouts; explicit heights avoid Safari ignoring `min-height` on native selects. The final 156-render production matrix reports only the reviewed inline-prose exception. | tags, brand overlap, Steam, Wishlist | DONE_WITH_DIFF |
...
| R12-PERF-003 | HIGH | Opening many VN tabs kept one download-status transport alive per hidden tab, while each polling or SSE consumer independently loaded durable jobs and enriched entity names. Hidden tabs now close polling, pending requests, and SSE without touching the server-owned job; visibility restores an immediate current snapshot. Concurrent polling and stream consumers coalesce one database/enrichment build. A stock batch regression advances from 1/10 to 7/10 while hidden and resumes at 7/10 without a cancellation request. Build and the final 9,693-test coverage suite pass at exact 100/100/100/100. | `src/components/DownloadStatusBar.tsx`, `src/lib/download-status-payload.ts`, download-status routes and tests | DONE_WITH_DIFF |
| R12-PERF-004 | MEDIUM | The character filter toolbar exposed many dynamic URL states at once. Next.js prefetched every visible chip, producing unnecessary server renders and WebKit-cancelled RSC requests when leaving the page. Speculative prefetch is disabled only on those filter chips, while ordinary click navigation remains intact. Local and deployed WebKit checks observe zero idle filter requests, exactly one request after clicking Main, a successful Characters-to-Staff transition, and no console or page error. | `src/app/characters/page.tsx`, WebKit production tour | DONE_WITH_DIFF |
...
| R12-TEST-001 | CRITICAL | The complete final coverage gate passes 9,693 tests at exactly 100 percent statements, branches, functions, and lines, with no exclusions, threshold changes, ignored files, or assertion-only hacks. | `yarn test:cov`, coverage configuration, all tests | DONE_WITH_DIFF |
...
| R12-TEST-004 | MEDIUM | The first post-prefetch coverage run exposed two UI tests whose first lazy modal import was coupled to Testing Library's wall-clock timeout. The triggering tests now preload the real modal modules, keeping the actual components and interactions while moving module transformation outside the UI wait. Their 106-test focused run and the final 9,693-test instrumented run pass. | `tests/StockPanel.test.tsx`, `tests/AliceNetClient.test.tsx` | DONE_WITH_DIFF |
| R12-TEST-005 | MEDIUM | Instrumented execution exposed the cover-picker trigger's pointer/focus prefetch still resolving after its jsdom environment had been destroyed. The triggering test now preloads the real picker module without mocking the component or extending teardown timing. The focused test and final 9,693-test coverage run complete without unhandled errors. | `tests/CoverPickerTrigger.test.tsx` | DONE_WITH_DIFF |
| R12-OPS-001 | CRITICAL | Production application code is deployed through the immutable release store, PostgreSQL readiness is green with pool max 10, nine migrations are current, systemd is active, and restart count is zero. The application role is non-superuser, database and application listeners are loopback-only, collection indexes are present, both backup timers and all checksums pass, and the latest dump restores successfully into a disposable database with 9 migrations, 3,317 VN, 167 collection rows, 8,058 offers, and 1,412 AliceNet rows. | production PostgreSQL, systemd, Nginx, backup operations | DONE_WITH_DIFF |
...
- Multi-tab production analysis reproduced 30 concurrent WebKit VN pages with
  ...
  final feature run passes 9,693 tests with 44,502 statements, 37,818 branches,
...
- The deployed WebKit character-filter check observes no speculative request
  while idle, one RSC request after the selected filter click, and no error
  while navigating to the staff page.
