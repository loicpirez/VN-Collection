...
| R12-PERF-004 | MEDIUM | The character filter toolbar exposes many dynamic URL states at once. Next.js prefetched every visible chip, producing unnecessary server renders and WebKit-cancelled RSC requests when leaving the page. Disable speculative prefetch only on those filter chips while preserving ordinary click navigation, then verify the Characters-to-Staff transition in WebKit and production access logs. | `src/app/characters/page.tsx`, WebKit production tour | TODO |
