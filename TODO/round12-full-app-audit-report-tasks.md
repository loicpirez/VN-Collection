...
| R12-SEC-001 | CRITICAL | Forwarded requests are no longer eligible for the direct-loopback shortcut. They require an enabled, constant-time proxy proof regardless of the public client IP; direct requests without forwarding metadata retain loopback access. Production Nginx now overwrites the proof after Basic Auth from a root-only snippet. | `src/lib/auth-gate.ts`, `src/lib/trusted-proxy.ts`, Nginx and production environment | DONE_WITH_DIFF |
