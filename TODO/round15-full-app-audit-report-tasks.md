...
| R15-RESP-003 | CRITICAL | The compact library still virtualized naturally variable-height card rows with one fixed estimated row height. Although both cards inside each rendered row aligned, changing the rendered window changed the document height by up to 81 pixels and shifted entire later rows while scrolling. Compact grids now keep the bounded 240-item API page in native CSS Grid flow with lazy images; wider grids retain estimated-row virtualization. This removes mobile row displacement without imposing a global card height or adding empty fixed-height card bodies. The browser contract now requires zero document-height drift, zero paired-edge divergence, and no estimated-row spacer on compact Chromium and WebKit grids. | `src/lib/virtual-grid.ts`, `src/components/LibraryClient.tsx`, `scripts/browser-interactions.mjs`, `scripts/responsive-audit.mjs` | DONE_WITH_DIFF |
...
- A deeper compact-grid trace found that the active release still changed the
  ...
  zero-pixel offset. Wider layouts continue to use virtualization.
