...
| R15-RESP-007 | CRITICAL | Automated WebKit reported equal item-card edges, but a later physical iPhone capture still showed the shorter card border ending above its shared CSS Grid row. The earlier evidence incorrectly assumed the capture predated deployment. Keep the card root as the direct grid item, retain intrinsic row sizing, and add an explicit full-track height fallback alongside stretch alignment so iOS repainting cannot return the shorter card to intrinsic height after content changes. Recheck all shared item-card surfaces and physical Safari before closure. | `src/components/VnCard.tsx`, item-card grids, physical iPhone Safari | IN_PROGRESS |
