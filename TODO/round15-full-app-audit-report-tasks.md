...
| R15-RESP-007 | CRITICAL | Automated WebKit reported equal item-card edges, but a later physical iPhone capture still showed the shorter card border ending above its shared CSS Grid row. Every library card now sits in an explicit stretching flex cell and fills that naturally sized CSS Grid row, avoiding Safari's cyclic direct-grid-item percentage-height behavior without imposing a global fixed/minimum height. Recheck all shared item-card surfaces and physical Safari before closure. | `src/components/VnCard.tsx`, `src/components/LibraryClient.tsx`, item-card grids, physical iPhone Safari | IN_PROGRESS |
