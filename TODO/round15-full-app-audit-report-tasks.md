...
| R15-RESP-007 | CRITICAL | Automated WebKit reported equal item-card edges, but a later physical iPhone capture still showed the shorter card border ending above its shared CSS Grid row. Every shared VN item-card surface now uses an explicit stretching flex cell and the card fills only that naturally sized CSS Grid row, avoiding Safari's cyclic direct-grid-item percentage-height behavior without imposing a global fixed/minimum height. Recheck library, wishlist, search, lists, series, relations, tags, reorder grids, and physical Safari before closure. | `src/components/VnCard.tsx`, shared item-card grids, physical iPhone Safari | IN_PROGRESS |
