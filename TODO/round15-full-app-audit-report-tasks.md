...
| R15-RESP-002 | CRITICAL | The mobile library grid stretched each semantic list cell to its row height but left the visual `VnCard` at its intrinsic height. Cards with one fewer metadata line ended 18 pixels above their sibling even though the next row used the taller track, creating visibly broken borders and empty inter-row gaps. Make the shared card fill only its own grid track, without a global fixed height, and audit visible card bottoms instead of the already-aligned wrapper cells. | `src/components/VnCard.tsx`, responsive production audit | IN_PROGRESS |
