...
| R15-RESP-002 | CRITICAL | The mobile library grid stretched each semantic list cell to its row height but Safari iOS could still paint the visual `VnCard` at its shorter intrinsic height. Removing the nested sizing wrapper made the real card root measurable and stopped the original overlap, but equal-height stretching was later rejected because it changed the compact card silhouette. The final intrinsic-height row contract is recorded in `R15-RESP-004`. Other card grids retain their existing behavior until independently audited. | `src/components/VnCard.tsx`, `src/components/LibraryClient.tsx`, `src/components/SortableGrid.tsx`, `src/components/ListReorderGrid.tsx`, `scripts/browser-interactions.mjs`, card-grid call sites, responsive production audit | DONE_WITH_DIFF |
...
| R15-RESP-004 | CRITICAL | A measured masonry attempt preserved intrinsic card height but made the two columns advance independently. Complete item cards therefore started at different vertical positions, reproducing the physical iPhone regression. Masonry measurement and one-pixel implicit rows are removed. The library now uses shared CSS Grid rows with start-aligned `VnCard` roots: both cards in each pair start at the same position, each border ends at its own intrinsic content height, and the next pair starts 12 or 16 pixels after the tallest card. WebKit and Chromium checks sample the full 167-card library through multiple scroll positions and require two columns, at most one pixel of paired top divergence, no overlap, no horizontal overflow, no document-height drift, and no flex or grid stretching on any card. | `src/components/VnCard.tsx`, `src/components/LibraryClient.tsx`, `scripts/browser-interactions.mjs`, item-card tests | DONE_WITH_DIFF |
| R15-RESP-005 | CRITICAL | Physical-phone review rejected the remaining unequal card borders after masonry removal: shared row starts alone still left shorter complete cards visibly offset from their partner. Library cards now stretch only to the tallest intrinsic card in their own CSS Grid row. No global, estimated, minimum, or fixed card height is introduced. Both top and bottom borders align within one pixel in WebKit and Chromium, the next row retains the configured 12 or 16 pixel gap, and the document height remains stable through scrolling. Other `VnCard` call sites retain their existing layout contract. | `src/components/VnCard.tsx`, `src/components/LibraryClient.tsx`, `scripts/browser-interactions.mjs`, item-card tests | DONE_WITH_DIFF |
...
| R15-RESP-007 | CRITICAL | A fresh post-correction production pass rechecked shared item cards across the library, wishlist, search, list detail, series detail, and tag detail in Chromium, Firefox, and WebKit; narrow phone, phone, landscape touch, and tablet layouts; and French, English, and Japanese. All 216 renders completed with zero card-edge divergence, page overflow, escaped or clipped control, undersized touch target, fixed-surface escape, runtime error, or locale mismatch. A separate active-control probe found zero overlap, outside-card action, obscured control, metadata collision, or quote-footer leak on the library and wishlist. The 12:33 physical screenshot records the earlier intrinsic-height release and predates the equal-row deployment at 22:45; the active release is retained instead of reintroducing the rejected iOS sizing wrapper. | production item-card surfaces, `scripts/responsive-audit.mjs`, active-control probe | VERIFIED_EXISTING |
...
- `R15-RESP-004` replaces the rejected compact masonry layout with shared
  intrinsic-height rows. WebKit and Chromium at 390 by 844 pixels both mounted
  all 167 semantic cards in two columns. Every two-card row started within one
  pixel, the next row started 12 to 13 pixels after the tallest preceding card,
  every card reported start alignment with zero flex growth, the document
  height remained stable through six scroll positions, and horizontal overflow
  remained zero. The complete interaction suite passed 29 of 29 scenarios,
  including desktop measured-row virtualization.
- `R15-RESP-005` closes the remaining full-card geometry defect without a
  fabricated height. Each library row is still measured from its own tallest
  intrinsic card, but both card roots now fill that row. Fresh 390 by 844
  WebKit and Chromium checks report zero-pixel top and bottom divergence,
  stable document height, the configured row gap, and no horizontal overflow.
  The full interaction suite passed 29 of 29 scenarios, the production build
  passed, and the instrumented suite passed 959 files and 10,096 tests at
  exactly 100 percent statements, branches, functions, and lines.
