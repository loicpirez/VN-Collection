...
| TESTA-002 | HIGH | Collection listing has no contract test proving private fields are absent from public-shaped responses. Add an exact response-body security test. | `tests/api-collection.test.ts`, `src/app/api/collection/route.ts` | TODO |
| TESTA-003 | HIGH | New stock browser, map, and place detail routes do not have route-level loading-state checks. Add file-presence QA and component smoke coverage. | `tests/qa-loading-states.test.ts`, `src/app/**/loading.tsx` | TODO |
...
| TESTA-008 | LOW | Rotated `SafeImage` rendering has no orientation regression coverage. Add component tests for portrait, landscape, and rotated framing calculations. | `tests/safe-image.test.tsx`, `src/components/SafeImage.tsx` | TODO |
