...
export function createPostgresSteamRepository(): SteamRepository {
  return {
    ...
    async applyPlaytime(applies) {
      ...
      const payload = JSON.stringify(applies.map((apply, index) => ({ ...apply, index })));
      const now = Date.now();
      return withPostgresTransaction(async (client) => {
        const result = await client.query<CountRow>(`
          WITH raw_input AS (
            SELECT value->>'vn_id' AS vn_id,
              (value->>'playtime_minutes')::bigint AS playtime_minutes,
              (value->>'index')::bigint AS input_index
            ...
            SET playtime_minutes = prior.playtime_minutes, updated_at = $2
            ...
            SET last_synced_minutes = prior.playtime_minutes, updated_at = $2
