...
export async function POST(req: NextRequest, ctx: { params: Promise<{ code: string }> }): Promise<NextResponse> {
  ...
  setKobeVnLink(code, normalizedVnId, normalizedVnId === null ? 'none' : 'manual');
  recordActivity({ kind: 'kobe.link', entity: 'alicesoft_kobe_stock', entityId: code, label: item.title, payload: { vn_id: normalizedVnId } });
