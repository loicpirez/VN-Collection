...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const items = await repository.listItemsForEgsResolve(batch);
  let processed = 0;
  try {
    for (const item of items) {
      const result = await resolveEgsForVn(item.vn_id, { allowSearch: true });
      if (result.game) {
        await repository.setEgsLink(item.code, result.game.id, 'auto', {
        ...
        await repository.setEgsLink(item.code, null, 'auto');
      }
      processed++;
    }
  } catch (err) {
    console.error('[alicenet/resolve-egs] upstream error:', (err as Error).message);
    const response = aliceNetApiError(err, 'EGS resolution failed.', 502, 'alicenet/resolve-egs');
    const body = await response.json() as { error: string };
    return NextResponse.json({ ...body, processed }, { status: 502 });
  }

  const { egs_pending } = await repository.countDownloadPending();
