...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json({ entries: await getActivityRepository().listForVn(id, 100) });
  } catch (err) {
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    ...
    const entry = await getGeneratedIdRepository().addManualActivity(id, body.text, at);
...
export async function DELETE(req: NextRequest, ctx: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    ...
    const ok = await getActivityRepository().deleteForVn(eid, id);
