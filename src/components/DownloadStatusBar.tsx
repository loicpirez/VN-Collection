...
export function DownloadStatusBar() {
  ...
  return (
    ...
    <div
      data-download-status-root
      className="fixed right-2 z-layer-status flex max-w-[calc(100vw-1rem)] flex-col items-end gap-2 sm:right-4 sm:max-w-sm"
      style={{
        bottom: 'calc(var(--quote-footer-height, calc(48px + env(safe-area-inset-bottom, 0px))) + 0.5rem)',
      }}
    >
      {retryingNow && (
          ...
          <header className="mb-2 flex items-center justify-between gap-2">
            <span className="text-[11px] font-bold uppercase tracking-widest text-muted">
              ...
              const stateIcon = finished
                ? failed
                  ? <><AlertTriangle className="h-3 w-3 shrink-0 text-status-dropped" aria-hidden /><span className="sr-only">{j.cancelled ? t.downloadStatus.kindCancelled : t.downloadStatus.kindError}</span></>
                  : <><CheckCircle2 className="h-3 w-3 shrink-0 text-status-completed" aria-hidden /><span className="sr-only">{t.downloadStatus.kindDone}</span></>
                : <><CloudDownload className="h-3 w-3 shrink-0 animate-pulse text-accent" aria-hidden /><span className="sr-only">{t.downloadStatus.kindRunning}</span></>;
              return (
