...
| R5-003 | Baseline | `yarn typecheck` passes before UX work. | repo |  | `logs/round5-final/typecheck.log`; verified at HEAD 4c90d53 — `yarn typecheck` exits 0 | FIXED_VERIFIED |
| R5-004 | Baseline | `yarn test --run` passes before UX work. | repo |  | `logs/round5-final/test.log`; verified at HEAD 4c90d53 — `yarn test --run` reports 2360/2360 passed across 235 files | FIXED_VERIFIED |
| R5-005 | Baseline | `yarn build` passes before UX work. | repo |  | `logs/round5-final/build.log`; verified at HEAD 4c90d53 — `yarn build` (Next 16 Turbopack) compiles every route | FIXED_VERIFIED |
| R5-006 | Runtime | `/character/c84419` has no RSC/function-boundary crash. | route `/character/c84419` |  | curl crash-marker check + Playwright route check; verified at HEAD 4c90d53; `src/app/character/[id]/page.tsx` is a clean async server component returning notFound() for missing ids | FIXED_VERIFIED |
| R5-007 | Runtime | `/character/c90980` has no RSC/function-boundary crash. | route `/character/c90980` |  | curl crash-marker check + Playwright route check; verified at HEAD 4c90d53; same route handler as R5-006 | FIXED_VERIFIED |
| R5-008 | Runtime | `/character/c69497` has no RSC/function-boundary crash. | route `/character/c69497` |  | curl crash-marker check + Playwright route check; verified at HEAD 4c90d53; same route handler as R5-006 | FIXED_VERIFIED |
| R5-009 | Runtime | `/staff/s12799` has no RSC/function-boundary crash. | route `/staff/s12799` |  | curl crash-marker check + Playwright route check; verified at HEAD 4c90d53; `src/app/staff/[id]/page.tsx` mirrors the character handler pattern | FIXED_VERIFIED |
| R5-010 | Runtime | `/staff/s1073?scope=collection` has no RSC/function-boundary crash. | route `/staff/s1073?scope=collection` |  | curl crash-marker check + Playwright route check; verified at HEAD 4c90d53; staff handler reads scope=collection via searchParams | FIXED_VERIFIED |
| R5-011 | Runtime | `/producer/p604` has no RSC/function-boundary crash. | route `/producer/p604` |  | curl crash-marker check + Playwright route check; verified at HEAD 4c90d53; `src/app/producer/[id]/page.tsx` resolves async params | FIXED_VERIFIED |
...
| R5-021 | VN Toolbar | One coherent toolbar structure and button primitive is used. | `VnDetailActionsBar`, `/vn/v26180`, `/vn/v28032`, `/vn/v4327` |  | source proof + Playwright bbox checks; verified at HEAD 4c90d53; `src/components/VnDetailActionsBar.tsx` consumes the shared `<DetailToolbarButton>` primitive | FIXED_VERIFIED |
...
| R5-026 | Cover/Banner | Cover rotation works on the currently displayed cover, persists after refresh, and resets persistently. | `/vn/v26180`, `/vn/v28032` |  | Playwright rotate/reload/reset test; verified at HEAD 4c90d53; `vn.cover_rotation` persisted column + `PATCH /api/collection/[id]/cover { rotation }` documented in CLAUDE.md | FIXED_VERIFIED |
| R5-027 | Cover/Banner | Banner rotation works and persists. | `/vn/v26180`, `/vn/v28032` |  | Playwright rotate/reload/reset test; verified at HEAD 4c90d53; `vn.banner_rotation` persisted column + `PATCH /api/collection/[id]/banner { rotation }` | FIXED_VERIFIED |
...
| R5-031 | Media Menu | Media action menu opens in a portal outside clipping contexts. | `MediaGallery`, `/vn/v26180` |  | Playwright bbox not clipped; verified at HEAD 4c90d53; `src/components/MediaGallery.tsx` uses createPortal for the lightbox + kebab menu | FIXED_VERIFIED |
...
| R5-034 | Spoilers | `SpoilerReveal` desktop mouse click reaches a fully revealed, readable state. | `SpoilerReveal`, `/vn/v4327` |  | unit test + Playwright click assertion; verified at HEAD 4c90d53; `tests/spoiler-reveal.test.ts` pins the truth table (5 modes) | FIXED_VERIFIED |
...
| R5-048 | Tags | VNDB `/g` tree groups are scraped, cached, and shown, not hardcoded fake partial tree. | `vndb-tag-web-cache/parser`, `/tags?mode=vndb` |  | parser tests + source proof; verified at HEAD 4c90d53; `src/lib/vndb-tag-web-cache.ts` parses & caches the live /g tree | FIXED_VERIFIED |
...
| R5-063 | Characters Browse | `searchLocalCharacters` uses batched VA-language query rather than per-row SELECT. | `src/lib/db.ts` |  | performance/unit query-count test; verified at HEAD 4c90d53; `src/lib/db.ts:5082` `searchLocalCharacters` uses a batched VA-language query | FIXED_VERIFIED |
...
| R5-066 | Recommendations | Generic/common tags do not dominate visible main reasons. | `/recommendations` |  | unit test + Playwright visible-tags assertion; verified at HEAD 4c90d53; `src/lib/recommend.ts` distinguishes distinctive vs downweighted tags | FIXED_VERIFIED |
...
| R5-068 | Recommendations | Default, hidden-gems, classics, and similar-to-vn modes differ meaningfully. | `/recommendations?mode=*` |  | fixture tests + QA; verified at HEAD 4c90d53; `src/lib/recommend.ts:9-16` defines mode=default | hidden-gems | classics | similar-to-vn with distinct scoring branches | FIXED_VERIFIED |
