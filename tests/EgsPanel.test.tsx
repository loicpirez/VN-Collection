...
describe('EgsPanel', () => {
  ...
  it('loads the empty state on mount when no initial snapshot exists', async () => {
    const { container } = renderPanel({ initialGame: undefined, initialSource: undefined });
    const skeleton = container.querySelector('[data-egs-empty-state-skeleton]');
    expect(skeleton).toBeTruthy();
    expect(skeleton?.querySelectorAll('.h-11')).toHaveLength(2);
