...
describe('visual viewport anchor', () => {
  ...
  it('keeps the native fixed fallback when no visual viewport exists', () => {
    ...
    document.body.append(anchor);
    const view = render(<VisualViewportAnchor />);
    expect(anchor.dataset.visualViewportMode).toBe('fixed');
    expect(document.documentElement.style.getPropertyValue('--visual-viewport-anchor-shift')).toBe('');
    view.unmount();
  ...
  it('leaves an in-flow mobile footer in document order without scroll listeners', () => {
    const anchor = document.createElement('div');
    anchor.dataset.visualViewportAnchor = '';
    anchor.style.position = 'static';
    document.body.append(anchor);
    document.documentElement.style.setProperty('--visual-viewport-anchor-shift', '24px');
    const scrollSpy = vi.spyOn(window, 'addEventListener');
    const view = render(<VisualViewportAnchor />);
    expect(anchor.dataset.visualViewportMode).toBe('flow');
    expect(document.documentElement.style.getPropertyValue('--visual-viewport-anchor-shift')).toBe('');
    expect(scrollSpy).not.toHaveBeenCalled();
    view.unmount();
    expect(anchor.dataset.visualViewportMode).toBeUndefined();
    expect(anchor.style.position).toBe('static');
    anchor.remove();
  });

  it('does nothing when the quote footer is not mounted', () => {
    const view = render(<VisualViewportAnchor />);
    expect(document.documentElement.style.getPropertyValue('--visual-viewport-anchor-shift')).toBe('');
    view.unmount();
  });
